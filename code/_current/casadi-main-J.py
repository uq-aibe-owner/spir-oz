...
# -J put these at the start, can let everything later depend on them
# -J group all dicts for policies into one
...
i_pol = list(d_dim) # returns a list of the keys

i_reg = [  #  -J don't want dependance on an index
    "aus",
    "qld",
    "wld",
]

i_sec = [ #  -J don't want dependance on an index
    "agr",
    "for",
    #"min",
    #"man",
...
NREG = len(i_reg)       # number of regions # -J move below
NSEC = len(i_sec)        # number of sectors
NPOL = len(pol_S)       # number of policy types: con, lab, knx, #itm
...
#==============================================================================
#-----------function for returning index subsets of x for a pair of dict keys # -J with adaptive key function this can of course go
def sub_ind_x(
...
#==============================================================================
LBX = DM.ones(NVAR) * 1e-1 # -J again parameterize
